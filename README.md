...
- 데이터: 카카오 로컬 REST API (조회 실패 시 `mockData.js` 목업 데이터로 대체 가능)
...
브라우저에서 `http://localhost:5501/` (시작/로그인 화면) 접속 → 로그인(또는 "로그인 없이 둘러보기")하면 메인 화면(`main.html`)으로 이동.
...
`.env.local`을 열어 아래 3개 값을 채웁니다. 전부 서버 전용이라 브라우저로 내려가지 않고, `api/*.js` 서버리스 함수 안에서만 `process.env`로 읽습니다.
...
세 값 모두 `.env.local`(로컬)/Vercel Environment Variables(배포)에 채우기만 하면 끝입니다 — 별도 빌드 처리가 필요 없습니다.

### 로컬 테스트 (`/api` 포함)

- 이 프로젝트의 데이터 조회 기능(내 주변, 구글 리뷰, AI 분석)은 전부 `/api` 서버리스 함수를 거칩니다. [Vercel CLI](https://vercel.com/docs/cli)를 설치해 `vercel dev`로 실행하면 `.env.local`을 자동으로 읽습니다.
- `python -m http.server` / Live Server 같은 일반 정적 서버로는 `/api`가 동작하지 않아 위 기능들이 전부 실패합니다(정적 자산만 보고 싶을 때만 사용).

### 배포 (Vercel)

`.env.local`은 커밋되지 않으므로, Vercel 프로젝트 설정 → Environment Variables에 위 3개 키(`GOOGLE_PLACES_API_KEY`, `GEMINI_API_KEY`, `KAKAO_REST_API_KEY`)를 **직접 대시보드에서** 동일한 이름으로 등록해야 배포본에서도 동작합니다. `api/*.js`는 Vercel이 자동으로 서버리스 함수로 배포하므로 별도 빌드 단계가 없습니다.

## 프로젝트 구조

```
index.html                  시작(로그인) 화면
main.html                   메인 화면 (MENU / CART 탭) — 로그인 모달도 여기서 마운트
...
.env.local.example           API 키 3개 템플릿 (커밋됨)
...
components/ui/               화면 컴포넌트 (CategoryChip, RestaurantCard, DetailModal, BottomNav, StatusView, AuthModal, AuthWidget)
